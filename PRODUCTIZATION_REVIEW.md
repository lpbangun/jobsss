...
Reviewed tree identity: published base
`803135995782e36cbf9427ac903a2e26d2952383` (origin/main, pushed) plus the
B61–B64 independent-validator evidence corrections. Historical identities
preserved: intermediate `ba2123ef5f6f24bcf6ae994a125b67501b970785`,
intermediate `8b2db255e7868397d336f8b015c489552b84bf0e`, and superseded
pre-audit commit `0b962bee448133eff9db6c953894e4ba360db79e`. Node v22.22.3,
host linux/x64. No staged files. The final corrective SHA is supplied in the final response
and is not invented here.

**Fresh verdict: PASS**

Scope: this is the single authoritative reviewer verdict for the
B61–B64 independent-validator evidence gate. The reviewer added additive
B61–B64 acceptance on top of published base `8031359`, recorded genuine
pre-correction failures (the pinned validator output and both reports
lacked complete canonical validator JSON and independent payload/fuse/
linkedit/resource/SizeOfImage evidence), and finalized both reports with
the live frozen-command count. Product files were not edited by this
reviewer. Frozen B1–B60 coverage was not weakened; B61–B64 remain
additive.

## B1–B41

Frozen twelve-file command after this review's report finalization:
`# tests 66` `# pass 66` `# fail 0`, exit `0`. B1–B41 all passed on
...
bytes unchanged. B42–B45 and B48–B64 passed in-suite.
...
verification) uses maintained exact-version/checksum-pinned validators
invoked from verified wheels on an isolated `PYTHONPATH`:
...
The test-only orchestration (`tests/helpers/jobsss-native-validators.mjs`)
never imports `src/packaging.js`, the in-repo parser, injector internals, or
synthetic fixture generators: it shells the pinned Python validators with a
standalone script that computes every offset/range/hash itself. Exact
canonical validator JSON for darwin-x64, darwin-arm64, win-x64-before, and
win-x64-after is recorded in the next section. An unsupported non-certificate overlay (trailing bytes not exactly the Security certificate table) is
rejected before mutation or tool acquisition (B57). Single authoritative
plugin/runtime version **0.1.0** (`plugin.json`, `src/cli.js` help, and
release-manifest agree).

- darwin-arm64 official Node SHA-256
  `5d9d3872911e2340a43b707962e68143de8a4e8d54628845c0c4f2de1fb7cd5c`.
  macholib 1.16.3 canonical JSON (B61): `"hasCodeSignature":false`, NODE_SEA
  filesize 57 / vmsize 16384 / fileoff 86605824, __LINKEDIT fileoff
  86622208 / filesize 25166240. LC_CODE_SIGNATURE removed (postject
  `remove_signature`; Node SEA `codesign --remove-signature` then re-sign).
- darwin-x64 official Node SHA-256
  `edc0e47adde954e891939bb509a62accdad5f6b15f32ec56ed78f9d6b7dd7308`.
  macholib 1.16.3 canonical JSON (B61): `"hasCodeSignature":false`, NODE_SEA
  filesize 55 / vmsize 4096 / fileoff 88850432, __LINKEDIT fileoff
  88854528.
- win-x64 official `node.exe` SHA-256
  `780f44f2c53c108bae261ada21a525b4bfe733c020ac85e41bfe94479090ac9b`.
  pefile 2024.8.26 canonical JSON (B62): pre-injection SizeOfImage 89866240
  equals the independently computed value; after injection SizeOfImage
  90009600 equals the independently computed value, FileAlignment 512,
  SectionAlignment 4096, `"security":{"fileOffset":0,"size":0}`,
  overlaySize 0, overlayOffset null, exactly one added `RT_RCDATA`
  `NODE_SEA_BLOB` resource, original resources preserved, and the
  pre-injection Authenticode certificate bytes not restored. Matching-host
  Windows distribution requires re-signing afterward; this host does not
  re-sign.
...
## Canonical independent-validator JSON (B61–B64)

This section is the complete independent native-format validation record
required by B61/B62, emitted by the pinned validators for the
four required cases (darwin-x64 after injection, darwin-arm64 after
injection, win-x64 before injection, win-x64 after injection). Commands:

```bash
node --test --test-concurrency=1 --test-name-pattern='B61 pinned|B62 pinned' tests/jobsss-native-remediation.test.mjs
JOBSSS_NATIVE_CACHE=<cache> node /tmp/gen-canonical-json.mjs  # injects official inputs with B61/B62 blobs
python3 (pefile 2024.8.26 / macholib 1.16.3 / altgraph 0.17.4) -- INSPECT_PY <pe|macho> <artifact>
```

All offsets/ranges/hashes below are computed by the pinned validators' own
independent parsers and the standalone test-only orchestration script,
never by JobSSS production parsers or the injector:
...
Interpretation: 40 canonical `fileOffsetChecks` entries per Mach-O case cover
every file-offset-bearing structure — each segment file range and every
file-backed section, `LC_SYMTAB` symbol/string tables
...
and `LC_MAIN` `entryoff` — each with explicit offset/size/inBounds.
Segments are 0x1000-aligned, non-overlapping and ordered
...
`LINKEDIT.relocated`/`delta` are computed by comparing against the sibling
official-before file created by B61 (darwin-x64 delta 4096, darwin-arm64
delta 16384), not hardcoded. `LC_SYMTAB`/`LC_DYSYMTAB` offsets lie inside
the file; exports (export trie via `LC_DYLD_INFO`), function-starts, and
data-in-code linkedit data are present and in bounds; `LC_CODE_SIGNATURE`
is absent (`hasCodeSignature:false`); fuse is present and enabled `:1`;
`payloadLength`/`payloadSha256` equal the exact injected blob. For PE:
`computedSizeOfImageInputs` records the independently derived header-table
end/linked size plus every section's `memSize`, `sectionEnd`, and
`alignedSectionEnd`, and the resulting `computedSizeOfImage` equals the
header `SizeOfImage` both before (89866240) and after (90009600); the
complete resource inventory adds per-resource data `size` and `sha256`;
every original resource survives with exactly one `RT_RCDATA
NODE_SEA_BLOB` added; `certificateRestored` is decided by searching the
image for the exact original certificate bytes (SHA-256
`6baff12dc6d38b97807001bb1eb5ff191bf09badae9ee5e2890a135bc25453c5`, 15688
bytes) — present in the before file, absent after injection; the Security
directory is cleared to 0/0; and all section raw/virtual ranges are in
bounds.
...
Focused B61–B62 failed on published base `8031359` because the pinned
validator output and reports lacked the canonical evidence fields (frozen
gate baseline); B63/B64 had no recordable pass until the test-only
validator orchestration and report evidence were corrected. After those
corrections the live frozen-command result is `# tests 66` `# pass 66`
`# fail 0`, exit `0`.
...
auditor native-safety checks; B61–B64 canonical independent-validator JSON
including payload/fuse, Mach-O load-command bounds/relocated linkedit/
SYMTAB/DYSYMTAB/linkedit-data, and PE resources/security/computed
`SizeOfImage`.
