 ...
 * Uses the operating system account database for the current uid (through
 * os.userInfo) so common per-user install locations can still be discovered
 * without hardcoding a username and without reading system account files.
 */
function realHome() {
  if (typeof process.getuid === 'function') {
    try {
      const account = os.userInfo();
      if (typeof account.homedir === 'string' && account.homedir.startsWith('/')) {
