...
const TOOLS = [
  ...
  tool('tailor_resume', 'Create a source-linked canonical resume draft. format pdf requires local Chrome or Edge for searchable Letter PDF export under PLUGIN_DATA. contactEmail selects the application address; locationNote accepts a verified Open to … relocation phrase.', schema({ ...jobProfile, format: string, style: string, contactEmail: string, locationNote: string }, ['jobId', 'profileId'])),
  tool('revise_resume', 'Create a new source-backed resume draft by suppressing or preferring explicit claim IDs from the inspectable ledger; preserves prior drafts and never accepts free-form unsupported claims.', schema({ ...jobProfile, format: string, contactEmail: string, locationNote: string, excludeClaimIds: { type: 'array', items: string }, preferClaimIds: { type: 'array', items: string } }, ['jobId', 'profileId'])),
...
const HANDLERS = Object.freeze({
  ...
  inspect_resume_qa: domain.inspectResumeQa, get_score: domain.getScore,
