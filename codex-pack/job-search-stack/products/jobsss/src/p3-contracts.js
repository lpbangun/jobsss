...
export function validatePublicUrl(raw) {
  ...
  const privateIPv4 = /^(127\.|10\.|192\.168\.|0\.0\.0\.0$)/.test(hostName)
    || /^172\.(1[6-9]|2\d|3[01])\./.test(hostName);
  const nonPublicName = hostName === 'localhost'
    || hostName === '::1'
    || hostName === '[::1]'
    ...
    || (!hostName.includes('.') && !isIPv4);
  if (isIPv4 ? privateIPv4 : nonPublicName || hostName === '::1' || hostName === '[::1]') {
