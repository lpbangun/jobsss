...
export function releaseCommand(argv, { repoRoot = repoRootFromSource() } = {}) {
  ...
  if (nodeBinaryArg) {
    ...
    if (baseSha !== expectedSha) {
      throw new Error(
        `release: checksum/sha256 mismatch: the Node executable in use here ${process.execPath} (${baseSha}) is not the locked official Node ${lockPin.version} ${injectTargetId} executable (${expectedSha}); refusing to build from an unpinned base`
      );
    }
  } else {
    throw new Error(
      `release: target ${targetArg} is unavailable on the current platform (${process.platform}/${process.arch}); ` +
