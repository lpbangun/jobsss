# JobSSS sourcing — frozen evaluator (sourcing-v1)

**Status:** FROZEN at authoring time, before L1/L2 start.
...
**Supersedes:** nothing. `BENCHMARK.md` (B1–B70) and `tests/jobsss-*.test.mjs` stay frozen and
...
`evaluation/MANIFEST-sourcing-v1.json` carries the same commands machine-readably, plus every
...
4. `node evaluation/freeze-files.mjs --verify` passes: every authored file still hashes to the frozen value.

## 6. Harness defects and limits (recorded, not scored)

Full machine-readable list: `harnessDefects` in `evaluation/MANIFEST-sourcing-v1.json`. Summary:
...
- Freeze procedure: `node evaluation/freeze-files.mjs --write` (records sha256 for every manifest
  file except the manifest itself), then `node evaluation/freeze-files.mjs --verify` must pass.
