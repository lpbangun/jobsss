# JobSSS sourcing — frozen evaluator (sourcing-v2)

**Status:** RE-FROZEN by reviewer erratum E1 after product revision `93c9de3`; scoring, fixtures, assertions, and commands are unchanged from sourcing-v1.
**Owner:** reviewer (L0, Codevisor). Implementers, lanes, and the conductor must not edit
this file, anything under `tests/sourcing-*.test.mjs`, `tests/fixtures/sourcing/`,
`tests/helpers/sourcing-harness.mjs`, or `evaluation/**`.
**Supersedes:** sourcing-v1 only for this mission's final evaluation. The v1 rubric and manifest remain preserved as authored evidence. `BENCHMARK.md` (B1–B70) and `tests/jobsss-*.test.mjs` stay frozen and
...
`evaluation/MANIFEST-sourcing-v2.json` carries the same commands machine-readably, plus every
...
- **expected MCP tool names are derived from the plugin itself** (`./bin/jobsss mcp` +
  ...
  transcript must equal that set, every one prefixed `mcp_jobsss_`;
...
4. `node evaluation/freeze-files.mjs --manifest evaluation/MANIFEST-sourcing-v2.json --verify` passes: every authored v2 file still hashes to the frozen value.

## 6. Harness defects and limits (recorded, not scored)

Full machine-readable list: `harnessDefects` in `evaluation/MANIFEST-sourcing-v2.json`. Summary:
...
- Freeze procedure for sourcing-v2: `node evaluation/freeze-files.mjs --manifest evaluation/MANIFEST-sourcing-v2.json --write` (records sha256 for every v2 manifest
