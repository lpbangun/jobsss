# JobSSS sourcing — frozen evaluator (sourcing-v4)

**Status:** RE-FROZEN by reviewer erratum E3 after release revision `72dad4e`; E1/E2 remain incorporated. Scoring, fixtures, behavioral assertions, thresholds, and product surfaces are unchanged from sourcing-v1/v3.
**Owner:** reviewer (L0, Codevisor). E3 was identified by final-judgment session `20260915_105053_41ef97`. Implementers and lanes must not edit this file, `tests/sourcing-*.test.mjs`, `tests/fixtures/sourcing/`, `tests/helpers/sourcing-harness.mjs`, or `evaluation/**`.
**Supersedes:** sourcing-v3 only for this mission's final evaluation. The v1/v2/v3 rubric and manifest files remain preserved evidence. `BENCHMARK.md` (B1–B70) and `tests/jobsss-*.test.mjs` stay frozen and
...
`evaluation/MANIFEST-sourcing-v4.json` carries the same commands machine-readably, plus every
...
4. `node evaluation/freeze-files.mjs --manifest evaluation/MANIFEST-sourcing-v4.json --verify` passes: every authored v4 file still hashes to the frozen value.

## 6. Harness defects and limits (recorded, not scored)

Full machine-readable list: `harnessDefects` in `evaluation/MANIFEST-sourcing-v4.json`. Summary:
...
- **D10 / E3 — check #9 depended on the evaluator's ambient profile.** Sourcing-v3 selected the
  allowed Hermes-home root from `process.env.HERMES_HOME`, so identical valid evidence passed from a
  neutral shell but failed from another Hermes profile. V4 resolves the stable real root as
  `path.join(os.homedir(), '.hermes')`; the existing explicit `--hermes-home-root` override remains
  available. Evidence requirements, scoring, product behavior, and boundary claims are unchanged.
...
- Freeze procedure for sourcing-v4: `node evaluation/freeze-files.mjs --manifest evaluation/MANIFEST-sourcing-v4.json --write` (records sha256 for every v4 manifest
