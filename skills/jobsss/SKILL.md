...
| Materials | `tailor_resume`, `draft_cover_letter`, `save_answer`, `list_answers`, `match_answers` | Drafts cite stored proof candidates. `save_answer` requires owned proof ids and generates from their exact summaries (or accepts that exact wording), never arbitrary claims, invented metrics, auto-fill, or send. |
