...
const TOOLS = [
  ...
  tool('tailor_resume', 'Create an applicant resume with separate review metadata. format pdf exports native searchable PDF bytes under PLUGIN_DATA and returns document.path.', schema({ ...jobProfile, format: string }, ['jobId', 'profileId'])),
  ...
  tool('import_contact', 'Import an inline or PLUGIN_DATA-staged profile-owned contact record; arbitrary paths are rejected.', schema({ ...profile, name: string, email: string, company: string, role: string, relationship: string, notes: string, source: string, text: string, content: string, path: string, filePath: string, expectedRevision: { type: 'integer' } }, ['profileId'])),
