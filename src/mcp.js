...
const TOOLS = [
  ...
  tool('import_job_url', 'Record a public http/https job URL for local review; rejects file and private URLs.', schema({ ...profile, url: string, expectedRevision: { type: 'integer' } }, ['profileId', 'url'])),
  tool('list_jobs', 'List profile-owned imported and discovered jobs.', schema(profile, ['profileId'])),
  tool('create_saved_search', 'Create a profile-owned offline public-ATS saved search using staged PLUGIN_DATA input.', schema({ ...profile, name: string, adapter: string, config: { type: 'object' }, minFit: { type: 'number' }, expectedRevision: { type: 'integer' } }, ['profileId', 'name', 'adapter'])),
  ...
  tool('daily_discovery', 'Run all profile saved searches offline; discoveries remain database-only until saved or pursued.', schema({ ...profile, expectedRevision: { type: 'integer' } }, ['profileId'])),
