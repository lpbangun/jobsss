...
const TOOLS = [
  ...
  tool('list_resume_design_variants', 'Read generated resume design variants and the recorded local preference for one content revision.', schema({ ...profile, variantGroupId: string }, ['profileId', 'variantGroupId'])),
  tool('select_resume_design', 'Record a local preference for one rendered resume design. This does not attest that the resume was submitted or used and does not record an application outcome.', schema({ ...profile, artifactId: string, expectedRevision: { type: 'integer' } }, ['profileId', 'artifactId'])),
  ...
  tool('update_profile', 'Update local preferences or correct the current resume identity with resumeIdentity.name and/or resumeIdentity.email.', schema({ ...profile, preferences: { type: 'object' }, name: string, resumeIdentity: { type: 'object', properties: { name: string, email: string } }, expectedRevision: { type: 'integer' } }, ['profileId'])),
  tool('archive_profile', 'Reversibly archive one explicit profile. Requires profileId and the current expectedRevision; retains all data and records an audit entry. Archived profiles cannot be used for search or scoring until restored.', schema({ ...profile, reason: string, expectedRevision: { type: 'integer' } }, ['profileId', 'expectedRevision'])),
  tool('restore_profile', 'Restore one previously archived profile. Requires profileId and the current expectedRevision; records an audit entry.', schema({ ...profile, expectedRevision: { type: 'integer' } }, ['profileId', 'expectedRevision'])),
...
const HANDLERS = Object.freeze({
  ...
  select_resume_design: domain.selectResumeDesign, get_score: domain.getScore,
  update_profile: domain.updateProfile, archive_profile: domain.archiveProfile,
  restore_profile: domain.restoreProfile, add_proof_point: domain.addProofPoint,
  ...
  update_task: domain.updateTask, tailor_resume: domain.tailorResume,
